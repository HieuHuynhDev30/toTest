...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    # return int(split_invoice_number[0]), int(split_invoice_number[1])


print(parse_invoice_number('2000-1555'))
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    """
    invoice_year, invoice_id = parse_invoice_number(invoice_number)
    if invoice_year == get_year():
        invoice_id += 1
    else:
        invoice_year = get_year()
        invoice_id = 1
    return f'{invoice_year}-{invoice_id:04d}'
...
def record_invoice(invoice_file: TextIO,
    ...
    :param last_line_ptr: The position of the start of the last line
        in the field. This will be obtained by the previous call to `record_invoice`.
    :return: The position of the start of the last line in the file. This can be used in subsequent
        calls to `record_invoice`.
    """
    invoice_file.seek(last_line, SEEK_SET)
    last_row = ''
    for index, row in enumerate(invoice_file):
        print('*', end='')
        last_row = row
    if last_row:
...
with open(data_file, 'r+', newline='') as csv_file:
    last_line = record_invoice(csv_file, 'Mac Donald', 123.456)
    last_line_1 = record_invoice(csv_file, 'Apple', 1000.75, last_line)
    record_invoice(csv_file, 'Microsoft', 7000.45, last_line_1)
